function [] = sphericalClustering3groupsModified(  )
%sphericalClustering( )
...
%  end
 ...
 unitVectors(:,5) = [-0.9832;-0.1826];
 unitVectors(:,6) = [0.1622;0.9868];
 unitVectors(:,7) = [-0.2896 ; 0.9572 ];
 unitVectors(:,8) = [0.9846 ; 0.1749 ];
...
noOfVectorsInEachGroup =floor( totalVectors/k);
...
for i =1:1:totalVectors
    labelArray(1, i ) = classLabel;
        count = count +1;
       if count == noOfVectorsInEachGroup 
...
fp = fopen('J:\Matlab\System Simulation Lab\E16-SphericalClustering\vectors.txt','w');
