function [ track ] = gradientDescent( x,y,muX,muY,sigmaX,sigmaY,maxItr,alpha) 
...
%       hold on;
%       gradientDescent( x,y,0,0,1,1,15,[.2,.3])

    point=[max(x);max(y)];
    mu=[muX;muY];
    sigma=[sigmaX^2 0;0 sigmaY^2];
    %alpha=rand(1,2);%;             %this random vector is used to generate the co-ordinates of the next generation
    %alpha=[0.5,0.3];
    alpha=alpha';
    %track=zeros(2,itr);
    for itr=1:1:maxItr
